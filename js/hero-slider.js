...
(function ($) {
  ...
  window.initHeroSlider = function (widgetId, bgImages, slideCount) {
    if (typeof Swiper === "undefined") {
      ...
      setTimeout(function () {
        window.initHeroSlider(widgetId, bgImages, slideCount);
      }, 100);
      return;
    }
    ...
    $(".hero-slider-" + widgetId + " .as-slider-background img")
      .eq(0)
      .addClass("currentForward");
    ...
    $(".hero-slider-" + widgetId + " .as-bar").html(dotsHTML);
    ...
    function updateTextContent(index) {
      var titleItems = $(
        ".hero-slider-" + widgetId + " .titles-widget .title-item"
      );
      var descItems = $(
        ".hero-slider-" + widgetId + " .desc-widget .desc-item"
      );
      ...
      $(".hero-slider-" + widgetId + " .titles-widget").css(
        "height",
        titleHeight + "px"
      );
      $(".hero-slider-" + widgetId + " .desc-widget").css(
        "height",
        descHeight + "px"
      );
      $(".hero-slider-" + widgetId + " .button-widget").css(
        "height",
        btnHeight + "px"
      );
    }
    ...
    $(document).on(
      "click",
      ".hero-slider-" + widgetId + " .swiper-slide",
      function (e) {
        var slideIndex = $(this).attr("data-index");
        if (slideIndex !== undefined) {
          swiper.slideToLoop(parseInt(slideIndex));
        }
      }
    );
  ...
  $(function () {
    $(".hero-slider").each(function () {
      ...
      var bgImages = $el.data("bg");
      var slideCount = $el.data("slides");

      if (widgetId && bgImages) {
        window.initHeroSlider(widgetId, bgImages, slideCount);
      }
    });
  });
